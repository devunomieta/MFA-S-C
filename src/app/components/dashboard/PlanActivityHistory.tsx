        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-tight">Activity History</h3>
                </div>
                <Badge variant="outline" className="font-bold border-gray-200 text-gray-400">Recent {activities.length}</Badge>
            </div>
            ...
            <div className="space-y-3">
                {activities.length === 0 ? (
                    <div className="py-12 text-center bg-gray-50 dark:bg-gray-900/50 rounded-[2rem] border border-dashed border-gray-200 dark:border-gray-800">
                        ...
                        <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">No activities yet</p>
                    </div>
                ) : (
                    activities.map((tx) => (
                        <div key={tx.id} className="group bg-white dark:bg-gray-950 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 hover:border-emerald-200 dark:hover:border-emerald-900/30 transition-all flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-4">
                                <div className={`size-10 rounded-xl flex items-center justify-center ${
                                    tx.type === 'deposit' || tx.type === 'transfer' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20' : 
                                    tx.type === 'fee' ? 'bg-amber-50 text-amber-600 dark:bg-amber-900/20' : 'bg-rose-50 text-rose-600 dark:bg-rose-900/20'
                                }`}>
                                    {tx.type === 'deposit' || tx.type === 'transfer' ? <ArrowDownLeft className="size-5" /> : 
                                     tx.type === 'fee' ? <Scale className="size-5" /> : <ArrowUpRight className="size-5" />}
                                </div>
                                <div>
                            ...
                            <div className="text-right">
                                <p className={`text-sm font-black ${
                                    tx.type === 'deposit' || tx.type === 'transfer' ? 'text-emerald-600' : 'text-gray-900 dark:text-white'
                                }`}>
