  ...
  const getActionDetails = (action: UIActivityType) => {
    switch (action) {
      ...
      default:
        return {
          text: "active on",
          icon: <LogIn className="size-4 text-slate-600" />,
          color: "bg-slate-50",
        };
    }
  };
        ...
        {activity && (
          ...
          >
            <div className="bg-white/95 backdrop-blur-xl border border-slate-200 p-4 rounded-2xl shadow-2xl flex items-center gap-4 max-w-sm ring-1 ring-black/5">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${getActionDetails(activity.action).color}`}
              >
              ...
              <div className="flex flex-col">
                <p className="text-sm font-bold text-slate-900 leading-tight">
                  ...
                  <span className="text-slate-500 font-medium">
                    {getActionDetails(activity.action).text}
                  </span>{" "}
                  {activity.plan}
                </p>
