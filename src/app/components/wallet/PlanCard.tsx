        ...
        >
            <div className="relative h-full p-6 flex flex-col justify-between text-white">
                ...
                <div className="flex justify-between items-start">
                    <div>
                        <p className={`text-[10px] uppercase tracking-wider mb-1 ${type === 'wallet' ? 'text-yellow-500 font-bold' : 'opacity-80'}`}>
                            {type === 'wallet' ? 'Main Wallet' : 'Savings Plan'}
                        </p>
                        <h3 className="font-bold text-lg leading-tight truncate max-w-[180px]">{name}</h3>
                    </div>
                    {/* Chip Visual */}
                    <div className={`w-10 h-8 rounded-md relative overflow-hidden flex items-center justify-center border ${type === 'wallet' ? 'bg-yellow-500/20 border-yellow-500/50' : 'bg-yellow-400/20 border-yellow-400/40'}`}>
                        <div className={`w-full h-[1px] absolute top-1/2 -translate-y-1/2 ${type === 'wallet' ? 'bg-yellow-500/50' : 'bg-yellow-400/40'}`} />
                        <div className={`h-full w-[1px] absolute left-1/2 -translate-x-1/2 ${type === 'wallet' ? 'bg-yellow-500/50' : 'bg-yellow-400/40'}`} />
                        <div className={`w-6 h-5 border rounded-sm ${type === 'wallet' ? 'border-yellow-500/50' : 'border-yellow-400/40'}`} />
                    </div>
                </div>

                {/* Balance */}
                <div className="my-2">
                    <p className="text-xs opacity-70 mb-1">{type === 'wallet' ? 'Available Balance' : 'Allocated Funds'}</p>
                ...
                <div className="flex justify-between items-end">
                    <div>
                        ...
                        {type === 'wallet' && (
                            <p className="text-[10px] text-yellow-500/80 uppercase tracking-widest">PREMIUM MEMBER</p>
                        )}
                    </div>
