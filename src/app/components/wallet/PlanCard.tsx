      ...
      <div className="relative h-full p-6 flex flex-col justify-between text-white">
        ...
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <p
                className={`text-[10px] uppercase tracking-wider ${type === "wallet" ? "text-yellow-500 font-bold" : "opacity-80"}`}
              >
          ...
          {/* Chip Visual */}
          <div
            className={`w-10 h-8 rounded-md relative overflow-hidden flex items-center justify-center border ${type === "wallet" ? "bg-yellow-500/20 border-yellow-500/50" : "bg-white/10 border-white/20"}`}
          >
            <div
              className={`w-full h-[1px] absolute top-1/2 -translate-y-1/2 ${type === "wallet" ? "bg-yellow-500/50" : "bg-white/20"}`}
            />
            <div
              className={`h-full w-[1px] absolute left-1/2 -translate-x-1/2 ${type === "wallet" ? "bg-yellow-500/50" : "bg-white/20"}`}
            />
            <div
              className={`w-6 h-5 border rounded-sm ${type === "wallet" ? "border-yellow-500/50" : "border-white/20"}`}
            />
          </div>
        </div>
        ...
        <div className="my-2">
          <p className="text-xs opacity-70 mb-1">
            {type === "wallet" ? "Available Balance" : "Current Balance"}
          </p>
          <div
            className={`text-3xl font-mono font-bold tracking-tight ${type === "wallet" ? "text-yellow-50 " : ""}`}
          >
            {currency}
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-tr pointer-events-none ${type === "wallet" ? "from-yellow-500/0 via-yellow-500/5 to-yellow-500/0" : "from-white/0 via-white/5 to-white/0"}`}
        />
      </div>
    </motion.div>
  );
