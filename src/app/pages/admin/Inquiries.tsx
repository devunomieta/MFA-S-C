...
export default function AdminInquiries() {
  ...
  const deleteInquiry = async () => {
    if (!deleteId) return;
    try {
      const { error } = await supabase.from("contact_inquiries").delete().eq("id", deleteId);
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        {/* List Side */}
        <div
          className={`${selectedInquiry ? "lg:col-span-5" : "lg:col-span-12"} space-y-6 transition-all duration-500`}
        >
            ...
            <CardHeader className="bg-white border-b border-slate-100 py-6">
              <div className="flex flex-col md:flex-row md:items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant="outline"
                    className="h-11 px-4 bg-slate-50 text-slate-600 border-slate-200"
                  >
                ...
                {loading ? (
                  ...
                  filteredInquiries.map((inquiry) => (
                    ...
                    >
                      ...
                      <p className="text-sm font-bold text-slate-700 truncate mb-2">
                        {inquiry.subject}
                      </p>
                      <div className="flex items-center justify-between text-[11px] text-slate-400 font-medium">
                        ...
                        <ChevronRight
                          className={`size-4 transition-transform ${selectedInquiry?.id === inquiry.id ? "rotate-90 text-emerald-600" : "group-hover:translate-x-1"}`}
                        />
                      </div>
                    </div>
                  ))
