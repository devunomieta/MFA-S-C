      ...
      if (tx.type === "withdrawal" && action === "reject" && tx.plan_id) {
        ...
        if (refundError) {
          ...
          if (up) {
            await supabase
              .from("user_plans")
              .update({ current_balance: up.current_balance + Number(tx.amount) })
              .eq("user_id", tx.user_id)
              .eq("plan_id", tx.plan_id);
          }
        }
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            description: rejectReason
              ? `${selectedDeposit.description} - Rejected: ${rejectReason}`
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8 text-gray-400 hover:text-blue-600"
                      onClick={() => contactUser(tx.profile?.email)}
                      title="Contact User"
                    >
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8 text-gray-400 hover:text-blue-600"
                      onClick={() => contactUser(loan.profile?.email)}
                      title="Contact User"
                    >
                      <Mail className="h-4 w-4" />
                    </Button>
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="all" className="mt-6">
          <Card>
            <CardContent className="pt-6">{renderTransactionTable(transactions)}</CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="deposits" className="mt-6">
          ...
          <Card>
            <CardContent className="pt-6">
              {renderTransactionTable(transactions, "deposit")}
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="loans" className="mt-6">
          ...
          <Card>
            <CardContent className="pt-6">{renderLoanTable()}</CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="withdrawals" className="mt-6">
          ...
          <Card>
            <CardContent className="pt-6">
              {renderTransactionTable(transactions, "withdrawal")}
            </CardContent>
          </Card>
        </TabsContent>
        <TabsContent value="revenue" className="mt-6">
          ...
          <Card>
            <CardContent className="pt-6">{renderRevenueTable(transactions)}</CardContent>
          </Card>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Determine how to handle this rejected deposit.</DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            <div className="space-y-4">
              <div className="flex items-start space-x-3 p-3 border rounded-md cursor-pointer hover:bg-gray-50 bg-gray-50">
                ...
                />
                <div>
                  <Label htmlFor="option-reject" className="font-semibold cursor-pointer">
                    Standard Reject
                  </Label>
                  <p className="text-xs text-gray-500 mt-1">
                    Funds were NOT received. Mark transaction as failed.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 p-3 border rounded-md cursor-pointer hover:bg-gray-50 border-emerald-100 bg-emerald-50">
                ...
                <div className="w-full">
                  <Label
                    ...
                    className="font-semibold cursor-pointer text-emerald-900"
                  >
                    Redirect to General Wallet
                  </Label>
                  <p className="text-xs text-emerald-700 mt-1">
                    Funds WERE received, but plan/context was wrong. Credit the user's General
                    Wallet instead.
                  </p>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className={
                rejectAction === "refund_wallet" ? "bg-emerald-600 hover:bg-emerald-700" : ""
              }
