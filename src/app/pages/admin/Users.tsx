                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          ...
                          title={
                            user.is_superadmin ? "Superadmins cannot be wiped" : "Wipe user data"
                          }
                        >
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            const response = await fetch(
              `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/system-purge-handler`,
              {
                method: "POST",
                headers: {
      ...
      >
        <div className="space-y-4 pt-4">
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">
            <strong>Warning:</strong> This will permanently delete all transactions, plans, and
...
function AdminWipePanel() {
  ...
  const handleWipe = async () => {
    ...
    setWiping(true);
    try {
      // console.debug("Invoking Edge Function with raw fetch: system-purge-handler");
      ...
      // console.debug("Fetching URL:", functionUrl);
      ...
      if (
        ...
        errorMessage =
          "Network Error: Could not reach the Edge Function. This usually means an adblocker or a network proxy is blocking the request to Supabase.";
      }
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="flex items-center gap-2 p-3 border rounded bg-white cursor-pointer hover:border-red-300">
                <input
                  type="radio"
                  checked={scope === "non-admin"}
                  onChange={() => setScope("non-admin")}
                />
                <div>
                  <div className="text-sm font-medium">Non-Admin Users</div>
                  <div className="text-xs text-slate-500">
                    Only removes users without admin privileges.
                  </div>
                </div>
              </label>
              <label
                className={`flex items-center gap-2 p-3 border rounded bg-white cursor-pointer hover:border-red-300 ${!isSuperadmin ? "opacity-50 grayscale pointer-events-none" : ""}`}
              >
                <input
                  type="radio"
                  checked={scope === "all-except-super"}
                  onChange={() => setScope("all-except-super")}
                  disabled={!isSuperadmin}
                />
                <div>
                  <div className="text-sm font-medium">All except Superadmin</div>
                  <div className="text-xs text-slate-500 text-red-600">
                    Removes everyone, including other Admins.
                  </div>
                  {!isSuperadmin && (
                    <div className="text-[10px] text-amber-600">Requires Superadmin status.</div>
                  )}
                </div>
              </label>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <label className="flex items-center gap-2 p-3 border rounded bg-white cursor-pointer hover:border-red-300">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    Auth credentials remain. Users can still log in but see a fresh account.
                  </div>
                </div>
              </label>
              <label className="flex items-center gap-2 p-3 border rounded bg-white cursor-pointer hover:border-red-300">
                ...
                <div>
                  <div className="text-sm font-medium text-red-600 font-bold">
                    Complete Auth Removal
                  </div>
                  <div className="text-xs text-slate-500">
                    Wipes everything, including the ability to log in. Users must register again.
                  </div>
                </div>
              </label>
            </div>
