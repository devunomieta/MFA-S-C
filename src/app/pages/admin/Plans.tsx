...
export function AdminPlans() {
    ...
    async function fetchPlans() {
        ...
        if (plansError) {
            ...
            const plansWithCount = plansData?.map((p: any) => ({
                ...p,
        ...
        setConfirmAction({
            ...
            action: async () => {
                ...
                if (error) {
                    ...
                    toast.success(`Success! Processed: ${data.processed}, Arrears Recorded: ${data.arrears_created}`);
                    fetchPlans();
                }
    ...
    const handleSubmit = async () => {
        ...
        if (error) {
            ...
            toast.success(editingPlan ? "Plan updated" : "Plan created successfully! Pending administrative approval.");
        ...
        <div className="space-y-8 animate-in fade-in duration-700">
            <AdminPageHeader
                ...
                actions={
                    ...
                    >
                        {isAutoSaving ? <Activity className="w-5 h-5 mr-2 animate-spin" /> : <TrendingUp className="w-5 h-5 mr-2" />}
            ...
            <AdminActionAuthModal 
                ...
                title={pendingAction?.toString().includes('setIsSystemUnlocked') 
                    ? "Unlock Sensitive System Conduits" 
                    : (pendingAction?.toString().includes('setIsDialogOpen') ? "Authorize New Plan Creation" : "Authorize Plan Approval")}
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div className="flex items-center gap-4">
                            <div className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-xl border border-emerald-100 flex items-center gap-2">
                                ...
                                <span className="text-sm font-black uppercase tracking-tighter">{plans.length} Registered Plans</span>
                            </div>
                        </div>
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
                        <Table>
                            <TableHeader className="bg-gray-50/50">
                                <TableRow>
                                    <TableHead className="w-[30%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Plan Identity</TableHead>
                                    <TableHead className="w-[10%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Structure</TableHead>
                                    <TableHead className="w-[10%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Timeline</TableHead>
                                    <TableHead className="w-[15%] text-center text-slate-400 font-black uppercase tracking-widest text-[10px]">Service Fee</TableHead>
                                    <TableHead className="w-[15%] text-center text-slate-400 font-black uppercase tracking-widest text-[10px]">Subscribers</TableHead>
                                    <TableHead className="w-[10%] text-center text-slate-400 font-black uppercase tracking-widest text-[10px]">Status</TableHead>
                                    <TableHead className="text-right w-[10%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Actions</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {loading ? (
                                    <TableRow><TableCell colSpan={6} className="text-center h-24 text-gray-500">Loading...</TableCell></TableRow>
                                ) : plans.filter(p => !p.name.includes('Wallet') && !p.name.includes('Payouts')).length === 0 ? (
                                    <TableRow><TableCell colSpan={6} className="text-center h-24 text-gray-500">No standard plans found.</TableCell></TableRow>
                                ) : (
                                    plans.filter(p => !p.name.includes('Wallet') && !p.name.includes('Payouts')).map((plan) => (
                                                ...
                                                <div className="flex flex-col items-center">
                                                    <span className="font-extrabold text-slate-900">
                                                        {plan.service_charge_type === 'percentage' ? `${plan.service_charge_percentage}%` : 
                                                         ...
                                                         formatNaira(plan.service_charge_fixed || plan.service_charge || 0)}
                                                    </span>
                                                    <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{plan.service_charge_is_recurring ? 'Monthly' : 'One-time'}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                    ...
                    {isSuperadmin && (
                        <div className="mt-12 space-y-6 animate-in slide-in-from-bottom-4 duration-1000">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-black text-slate-900 tracking-tight">Internal System Conduits</h2>
                                    <p className="text-sm font-medium text-slate-500">Core financial infrastructure for payouts and maturing funds. Restricted access.</p>
                                </div>
                            </div>
                            ...
                            {!isSystemUnlocked ? (
                                <div className="bg-white border-2 border-dashed border-slate-200 rounded-[2rem] p-12 text-center space-y-4">
                                    ...
                                    <div className="max-w-md mx-auto">
                                        ...
                                        <p className="text-sm text-slate-500 mb-6">Viewing internal conduits requires active identity verification via Administration PIN. Standard actions are disabled for these conduits.</p>
                                        <Button 
                                            onClick={() => {
                                ...
                                <div className="rounded-xl border border-gray-200 bg-emerald-50/10 shadow-sm overflow-hidden animate-in fade-in duration-500">
                                    <Table>
                                        <TableHeader className="bg-slate-950">
                                            <TableRow>
                                                <TableHead className="w-[40%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Plan Identity (System)</TableHead>
                                                <TableHead className="w-[15%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Structure</TableHead>
                                                <TableHead className="w-[15%] text-slate-400 font-black uppercase tracking-widest text-[10px]">Timeline</TableHead>
                                                <TableHead className="w-[15%] text-center text-slate-400 font-black uppercase tracking-widest text-[10px]">Service Fee</TableHead>
                                                <TableHead className="w-[15%] text-center text-slate-400 font-black uppercase tracking-widest text-[10px]">Subscribers</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody className="bg-white">
                                            {plans.filter(p => p.name.includes('Wallet') || p.name.includes('Payouts')).map((plan) => (
                                                <TableRow key={plan.id} className="hover:bg-slate-50/50">
                                                    ...
                                                    <TableCell><span className="capitalize px-2 py-0.5 rounded-full bg-slate-100 text-[10px] font-bold">INTERNAL</span></TableCell>
                                                    <TableCell className="text-slate-500 font-medium">FIXED</TableCell>
                                                    <TableCell className="text-center font-black text-slate-900">₦0.00</TableCell>
                                                    <TableCell>
                                                        <div className="flex items-center justify-center gap-2 text-slate-700 font-black bg-white border border-slate-200 rounded-xl py-2 px-3 shadow-sm mx-auto w-fit">
                                                            <Users className="w-4 h-4 text-emerald-500" />
                                                            {plan.subscriber_count as number}
                                    ...
                                    <div className="p-4 bg-emerald-50 border-t border-emerald-100 flex items-center gap-3">
                                        ...
                                        <p className="text-[10px] font-bold text-emerald-800 uppercase tracking-widest">Read-Only Conduits: Standard administrative actions are disabled to prevent core system failure.</p>
                                    </div>
                                </div>
                            )}
                        </div>
