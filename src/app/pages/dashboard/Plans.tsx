...
const PlanCardGrid = ({ items, type, myPlans }: {
    items: any[],
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
            {items.length === 0 ? (
                ...
                items.map((item, index) => {
                    ...
                    return (
                        <Card key={item.id || `plan-${index}`} className="group hover:shadow-2xl transition-all duration-500 border-gray-100 dark:border-gray-800 overflow-hidden flex flex-col h-full bg-white dark:bg-gray-950 shadow-sm relative hover:-translate-y-1">
                            {/* Decorative accent */}
                            <div className={`absolute top-0 left-0 w-full h-1.5 ${
                                plan.type === 'marathon' ? 'bg-emerald-500' :
                                plan.type === 'sprint' ? 'bg-blue-500' :
                                plan.type === 'anchor' ? 'bg-indigo-600' :
                                plan.type === 'daily_drop' ? 'bg-cyan-500' :
                                plan.type === 'step_up' ? 'bg-teal-600' :
                                ...
                                <div className="flex justify-between items-start">
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {type === 'active' ? (
                                                <Badge className={`text-[10px] font-black px-2 py-0.5 border-none ${
                                                    item.status === 'matured' ? 'bg-amber-100 text-amber-700' :
                                                ...
                                                <Badge variant="outline" className="text-[10px] uppercase font-black text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/30 bg-emerald-50 dark:bg-emerald-900/10 tracking-wider px-2 shadow-sm">
                                                    {plan.type === 'marathon' ? 'Endurance' :
                                                     plan.type === 'sprint' ? 'Velocity' :
                                                     plan.type === 'anchor' ? 'Stability' :
                                                     plan.type === 'daily_drop' ? 'Consistency' :
                                                     plan.type === 'step_up' ? 'Progress' :
                                                     plan.type === 'monthly_bloom' ? 'Harvest' :
                                                     'Community'}
                                                </Badge>
                                            )}
                                        </div>
                            ...
                            <CardContent className="flex-1 space-y-5 pt-4">
                                ...
                                >
                                    {type === 'active' || isJoined ? 'Manage Plan' : `Join ${plan.name.replace(/^The\s+/i, '')}`}
                                </Button>
                            </CardContent>
                        </Card>
                    );
...
export function Plans() {
    ...
    const activePlansList = myPlans
        .filter(p => (p.status === 'active' || p.status === 'matured' || p.status === 'pending_activation' || p.status === 'completed'))
        .sort((a, b) => {
    ...
    return (
        <div className="space-y-10">
            <div className="space-y-1">
                <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Savings Plans</h1>
                <p className="text-gray-500 dark:text-gray-400 font-medium">Choose a plan that suits your financial goals.</p>
            </div>
            ...
            <Tabs defaultValue="available" className="w-full">
                ...
                <TabsContent value="available" className="pt-8">
                    <PlanCardGrid
                        items={availablePlans.filter(ap => !myPlans.some(mp => mp.plan_id === ap.id && mp.status !== 'cancelled'))}
                        type="available"
                        myPlans={myPlans}
                    />
                </TabsContent>
