...
export function DailyDropPlanCard({
  plan,
  userPlan,
  onJoin,
  onRefresh,
  ...
  const [joinAmount, setJoinAmount] = useState<string>(
    plan.min_amount?.toString() || plan.fixed_amount?.toString() || "500",
  );
  ...
  const effectiveDaysPaid =
    fixedAmount > 0 ? Math.max(daysPaid, Math.floor(totalSaved / fixedAmount)) : daysPaid;
  ...
  const getNextDue = () => {
    ...
    const diffDays = Math.ceil(
      (coverageEndDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24),
    );
    ...
    return coverageEndDate.toLocaleDateString("en-GB", {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
    ...
    return (
      <Card className="flex flex-col relative overflow-hidden bg-white dark:bg-gray-900 border-l-4 border-l-cyan-500 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="bg-cyan-50 dark:bg-cyan-900/20 p-4 rounded-xl border border-cyan-100 dark:border-cyan-800 space-y-3">
                ...
                >
                  {withdrawing
                    ? "Withdrawing..."
                    : metadata.withdrawn
                      ? `Withdrawn ${formatNaira(metadata.withdrawn_amount || fixedAmount * selectedDuration)}`
                      : "Withdraw to Wallet"}
                </Button>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                {effectiveDaysPaid > 0 && !isFinished && (
                  <div className="flex items-center gap-2 p-2 bg-blue-50 text-blue-700 rounded-md text-[10px] border border-blue-100 font-bold leading-tight">
                    ...
                    <span>
                      Advance Payment: {effectiveDaysPaid} Day{effectiveDaysPaid === 1 ? "" : "s"}{" "}
                      Covered
                    </span>
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-800">
                  <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-1 flex items-center gap-1">
                    <AlertTriangle
                      className={`w-3 h-3 ${nextDue.includes("Tomorrow") ? "text-emerald-500" : "text-amber-500"}`}
                    />{" "}
                    Next Due
                  </div>
                  <div
                    className={`text-sm font-bold mt-1 ${nextDue.includes("Tomorrow") ? "text-emerald-600" : "text-gray-900 dark:text-gray-100"}`}
                  >
                    {nextDue}
                  </div>
                </div>
              ...
              {isChangingAmount && (
                <div className="p-4 bg-cyan-50 dark:bg-cyan-900/20 rounded-lg border border-cyan-100 dark:border-cyan-800 space-y-3 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button size="sm" className="h-8 bg-cyan-600" onClick={handleChangeAmount}>
                      Save
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-8"
                      onClick={() => setIsChangingAmount(false)}
                    >
                      Cancel
                    </Button>
                  </div>
        ...
        <CardFooter className="flex flex-col gap-3 pt-2 pb-6">
          ...
          {(userPlan.status === "pending_activation" ||
            (userPlan.status === "completed" && metadata.withdrawn) ||
            (isFinished && metadata.withdrawn)) &&
            onLeave && (
              <Button
                variant="ghost"
      ...
      <CardHeader className="pb-4">
        <div className="flex justify-between items-start">
          <div>
            <Badge
              variant="secondary"
              className="mb-2 bg-cyan-50 text-cyan-700 border-cyan-100 hover:bg-cyan-100"
            >
              Daily Savings
            </Badge>
      ...
      <CardContent className="flex-1 space-y-6 pt-2 overflow-y-auto max-h-[60vh] custom-scrollbar">
        ...
        <div className="space-y-4 pt-2">
          <div className="p-3 bg-cyan-50 dark:bg-cyan-900/20 rounded-lg border border-cyan-100 dark:border-cyan-800">
            <h4 className="text-[10px] font-bold text-cyan-800 dark:text-cyan-400 uppercase tracking-wider mb-2">
              Rules & Features
            </h4>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-lg font-black text-gray-900 dark:text-white">
                        {formatNaira(parseFloat(joinAmount))}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold text-cyan-600 tracking-wider">
                  ...
                  <p className="text-xs text-amber-800 dark:text-amber-400 leading-relaxed">
                    ...
                    plan.service_charge_percentage === 100 ? (
                      <>
                        ...
                        <span className="font-bold">{formatNaira(parseFloat(joinAmount))}</span>{" "}
                        will be charged as service fees.{" "}
                        {plan.service_charge_is_recurring
                          ? `This fee will be deducted every ${plan.service_charge_interval_days} days.`
                          : "Future drops go 100% into your savings."}
                      </>
                    ) : plan.service_charge_type === "percentage" ? (
                      <>
                        Your first payment of{" "}
                        <span className="font-bold">
                          {formatNaira(
                            (parseFloat(joinAmount) * (plan.service_charge_percentage || 0)) / 100,
                          )}
                        </span>{" "}
                        {plan.service_charge_is_recurring
                          ? `and subsequent payments every ${plan.service_charge_interval_days} days`
                          : ""}{" "}
                        will be charged as service fees.
                      </>
                    ) : plan.service_charge_type === "fixed" ? (
        ...
        <AlertDialogContent className="rounded-2xl border-emerald-100 dark:border-gray-800">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="pt-2">
              This will archive your current {plan.name} record and let you start a fresh cycle.
              Your current balance will be preserved in your history.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2 pt-4">
